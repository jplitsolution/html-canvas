...
export default class GlobalErrorBoundary extends Component {
  ...
  componentDidCatch(error, info) {
    // Safely call logError if it exists
    try {
      const store = useStore.getState();
      if (typeof store.logError === 'function') {
        store.logError(this.props.name || 'App', error);
      } else {
  ...
  handleSafeMode = () => {
    try {
      const store = useStore.getState();
      if (typeof store.setSafeMode === 'function') {
        store.setSafeMode(true);
      }
    } catch (e) {
      console.warn('Safe mode not available');
    }
    this.setState({ hasError: false, error: null });
  };
      ...
      return (
        <div className="min-h-screen flex items-center justify-center p-8 bg-bg-base">
          <div className="max-w-md w-full glass rounded-xl p-8 text-center">
            ...
            <h2 className="text-xl font-semibold text-fg font-display mb-2">
