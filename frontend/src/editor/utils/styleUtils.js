 ...
 */
export function encodeNonAscii(html) {
 ...
 */
export function transformReactComponentsInHtml(html) {
  ...
  const replaceTag = (el, newTagName) => {
 ...
 */
export function injectStylesheetsIntoCanvas(editor) {
  ...
  // 2. Host Link Sheets
  Array.from(document.querySelectorAll('link[rel="stylesheet"]')).forEach((linkEl) => {
  ...
  Array.from(document.querySelectorAll('style')).forEach((styleEl, index) => {
 ...
 */
export function validateStylesheets(doc) {
  const missing = [];
 ...
 */
export function getActiveStylesheetsContent() {
  let cssText = '';
  try {
 ...
 */
export function runDevModeStylesValidation(doc) {
