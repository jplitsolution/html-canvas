let previewEl = null
let placerLabelEl = null
let activeBlock = null
let onOverCanvas = null

function isPointOverCanvas(editor, x, y) {
...
function movePreview(editor, clientX, clientY) {
  if (previewEl) {
...
function enhanceCanvasPlacer(editor, block) {
  ...
  const inner = placer.querySelector('.gjs-placeholder-int')
  if (!inner) return
...
function clearCanvasPlacer(editor) {
  const placer = editor.Canvas.getPlacerEl()
...
function createFloatingPreview(block) {
  const label = String(block.get('label') || block.get('id') || 'Block')
  const media = block.get('media')
...
function cleanupPreview(editor) {
  previewEl?.remove()
  previewEl = null
  activeBlock = null
...
/** Cursor-following ghost + in-canvas drop preview */
export function setupDragPreview(editor, onOverCanvasChange) {
