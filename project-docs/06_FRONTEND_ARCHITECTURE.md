...
- **Funnel Transition & Navigation Interception**: Click events inside the shadow root are intercepted by traversing the composition path (`composedPath()`). Actions like click buttons or image hotspots containing `data-action` (e.g. `SUBSCRIBE`) are captured and routed to the backend `/flow/transition` endpoint. If an anchor link is clicked that points to a valid internal campaign page step (like `HOME`, `OTP`, `CONFIRM`, etc.) without a system action, the runtime intercepts the click and updates the query parameter (`step`) to perform a client-side route transition without reloading the page.
