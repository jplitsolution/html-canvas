  ...
  async send(@Body() body) {
    ...
    if (body.visitId) {
      try {
        ...
        this.logger.error(`Error resolving provider for masking: ${err.message}`);
      }
    }

    const env = this.configService.get('environment') || 'development';
    const otpExposeTest = this.configService.get('otpExposeTest') || false;
