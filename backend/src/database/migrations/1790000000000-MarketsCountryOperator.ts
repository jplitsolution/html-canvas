...
/**
 * Introduces countries + operators master data, links campaigns via operator_id,
 * drops unique (country, operator) so many campaigns can share one market.
 */
export class MarketsCountryOperator1790000000000 implements MigrationInterface {
  ...
  private async safeQuery(queryRunner: QueryRunner, sql: string): Promise<void> {
    try {
      ...
      // ignore (idempotent / already applied)
    }
  }

  private deriveCountryCode(name: string): string {
  ...
  private deriveOperatorCode(name: string): string {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    if (isPostgres) {
      ...
      // Backfill countries / operators / operator_id from existing campaigns
      const rows: Array<{
        id: number;
        country: string;
        operator: string;
      ...
      // Deduplicate names per operator before unique index (append id on conflict)
      await this.safeQuery(
        queryRunner,
        `
        UPDATE "campaigns" c
